...
- Tests exist in `backend/tests/` (`conftest.py`, `test_smoke.py`, `test_fase5.py`, `test_fase6.py`, `test_fase8.py`, `test_fase9.py`, `test_fase10.py`, `test_fase12.py` — 38 tests). Run them from `backend/` with `pytest`.
...
**Fase 12 is done** (ADR-014, no schema change): reactive date cascade in the creation editor — changing any card's business days or start date recomputes the following cards' start dates via the new `POST /calendario/cascata` (one round-trip; `cascatearItens()` in `etapasEditorUtils.js`, wired into `EtapasEditor.jsx` with an in-flight sequence guard; a card without dias breaks the chain) — plus post-creation editing: `PATCH /etapas/{id}` (partial update; dias/data on a block member propagate to all members per ADR-009, nome/descricao stay individual), `PUT /projetos/{id}/etapas/ordem` (full id list → ordem = index + 1), and `ModalEditarEtapa.jsx` (state owned by `EtapasProjeto.jsx` as `edicaoIds`, opened by ✏️ `Pencil` buttons in `KanbanEtapas.jsx` and `TabelaEtapas.jsx`; block mode edits shared dias/data, per-member names and member order). **Fase 13 is planned but NOT started** (defined 06/07/2026, ADR-015; same "no phase without a direct command" rule). Fase 13 (**schema change → drop/recreate the `.db` + re-seed, ADR-001**): reintroduces stage dependencies removed by ADR-006 — a new `EtapaDependencia` table (blocked-by/blocking, **informational only, no auto-reschedule**, coexisting with delivery blocks), plus an interactive cronograma (drag to move `data_inicio`, resize to change `dias_uteis_esperados` via a new reverse-calendar `GET /calendario/dias-uteis`, drag-connector to link dependencies) and "Bloqueado por"/"Bloqueando" columns in the Tabela. The executable plan lives in [docs/features/plano-fases-12-13.md](docs/features/plano-fases-12-13.md). **Read that plan before executing either phase.**
