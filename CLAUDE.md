...
**Fases 12 and 13 are planned but NOT started** (defined 06/07/2026, ADR-014/ADR-015; same "no phase without a direct command" rule). Fase 12 (no schema change): reactive date cascade in the creation editor (any stage's business-days change recomputes its own end date and the following stages' start dates, chaining `data_fim` via the backend per ADR-008 + a new `POST /calendario/cascata`) plus post-creation editing of etapas/blocks — new `PATCH /etapas/{id}` (block-aware: date/prazo propagate to members), `PUT /projetos/{id}/etapas/ordem`, and a `ModalEditarEtapa.jsx` reachable from both the Kanban and the Tabela. Fase 13 (**schema change → drop/recreate the `.db` + re-seed, ADR-001**): reintroduces stage dependencies removed by ADR-006 — a new `EtapaDependencia` table (blocked-by/blocking, **informational only, no auto-reschedule**, coexisting with delivery blocks), plus an interactive cronograma (drag to move `data_inicio`, resize to change `dias_uteis_esperados` via a new reverse-calendar `GET /calendario/dias-uteis`, drag-connector to link dependencies) and "Bloqueado por"/"Bloqueando" columns in the Tabela. The executable plan lives in [docs/features/plano-fases-12-13.md](docs/features/plano-fases-12-13.md). **Read that plan before executing either phase.**
