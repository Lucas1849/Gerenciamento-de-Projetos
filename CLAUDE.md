...
- Tests exist in `backend/tests/` (`conftest.py`, `test_smoke.py`, `test_fase5.py`, `test_fase6.py` — 15 tests). Run them from `backend/` with `pytest`.
...
**Fase 7a is done** (Etapas tab is now first and the default in `PaginaProjeto.jsx`). **Fases 7b–11 are planned but NOT started** — the product owner explicitly ordered that no phase begins without his direct command. The executable plan lives in [docs/features/plano-fases-7-11.md](docs/features/plano-fases-7-11.md): 7b (multiple stage views: kanban/table/timeline/calendar with a pill sub-nav), 8 (N-etapa blocks: extend, remove a specific member, full undo), 9 (DELETE routes + trash buttons for gestões/projetos), 10 (date plausibility validation, DD/MM/YYYY), 11 (go-live/Hub integration, externally gated). **Read that plan before executing any of those phases.**
