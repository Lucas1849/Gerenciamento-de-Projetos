...
- Smoke tests exist in `backend/tests/` (`conftest.py` + `test_smoke.py`, 4 tests). Run them from `backend/` with `pytest`.
...
**Fases 3 and 4 are done** (Apoio Hub dark retheme + sidebar shell + responsiveness; TAP status toggle on the project page). **Fases 5 and 6 are planned but not started** — stage dates with business-day end-date calculation (workalendar), editable/reorderable stage cards at project creation (@dnd-kit), and interactive block deliveries. The approved plan, product decisions and per-phase status live in [docs/features/plano-fases-3-6.md](docs/features/plano-fases-3-6.md) — **read it before executing Fase 5 or 6**. Note: Fase 5 changes the DB schema (`Etapa.data_inicio`) and triggers the ADR-001 flow.
