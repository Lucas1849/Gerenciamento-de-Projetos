...
**Next step (Fase 2): seed the service catalog.** The `Servico`/`EtapaTemplate` tables exist but are **empty**, and the frontend has no UI to create services (by design — the catalog is diretoria-validated content, not user data). The plan is a `backend/app/seed_catalogo.py` script seeding the real catalog from [docs/features/catalogo-servicos.md](docs/features/catalogo-servicos.md) (real stages, expected business days, sources — already approved by the diretoria).
