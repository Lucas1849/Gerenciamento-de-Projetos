...
- **Nível 1 — galeria de gestões:** `App.jsx` lista as `Gestao` (semestres); ao entrar numa gestão, `KanbanFases.jsx` mostra os projetos num Kanban de **5 colunas** por `Projeto.fase` (`kickoff` → `andamento` → `finalizacao` → `ajustes` → `concluido`), com colunas no estilo do Hub (dot colorido + contagem, affordance fantasma "+ Novo projeto" em coluna vazia) e cards com chip do serviço, bloco GERENTE e avatares dos consultores (equipe derivada embutida pelo `GET /projetos/`).
