...
### ADR-020 — Abas na galeria de gestões (Gestões / Documentos importantes): links da área, dashboards adiados ao roadmap (Fase 18)

**Contexto:** o responsável pediu um menu horizontal de abas no padrão das já existentes na página do projeto, com Documentos importantes e Dashboards além da visão atual. **Revisão de 09/07/2026 (decisão do diretor, ainda em planejamento):** (1) os links de documentos são **da área** e ficam na **galeria de gestões**, não dentro de cada gestão junto ao Kanban de projetos; (2) a aba **Dashboards sai da fase** — os KPIs nunca foram levantados (comentário "por cima" do diretor) e o item volta ao [roadmap](../features/roadmap.md) até a diretoria defini-los.

**Decisão:**
- **Shell de abas** em `TelaGaleriaGestoes` (App.jsx) reusando `.tabs-container`/`.tab` (visual da `PaginaProjeto`), estado local não persistido (coerente com ADR-010): **Gestões** (a galeria atual, intocada, default) e **Documentos**. **Sem aba Dashboards** — nem placeholder.
- **Documentos = links nomeados para o Drive, da área como um todo** (formato confirmado pelo responsável em 09/07/2026, replicando a página de documentos que a área mantém no Notion — que mistura links gerais e de gestões específicas): nova tabela aditiva `Documento` (`id`, `nome`, `url`, `criado_em`), **sem FK de gestão nem de projeto** (decisão do diretor; vincular vira evolução se um dia pedirem filtro). CRUD leve em router próprio (`GET`/`POST /documentos`, `DELETE /documentos/{id}`). O piloto não tem storage de arquivos e a informação continua concentrada no Drive; upload real fica no roadmap (integração Hub).
- **Dashboards → roadmap:** a proposta-base (rota agregadora com termos/dias aditivos por projeto e serviço como métrica de atraso formalizado, sem lib de gráficos) foi transcrita para a seção "Métricas e dashboards de projetos" do roadmap como pauta do levantamento de KPIs com a diretoria.
...
**Status:** implementado (09/07/2026) — Fase 19 executada sob comando direto do responsável. Nota de implementação: tabela `EtapaLink` + rotas em `routes/etapas.py` (`POST /etapas/{id}/links`, `DELETE /etapas/{id}/links/{link_id}`); `links[]` em `EtapaResposta`; UI na seção "Entregas e demandas" do `ModalEditarEtapa.jsx` (lista + formulário inline, por membro em bloco), chip `Paperclip` de contagem nos cards do Kanban (abre o modal) e coluna "Links" na tabela; chips tonais `.chip-link-entrega`/`.chip-link-demanda`. Testes em `test_fase19.py`. Ver [../features/plano-fases-19-20.md](../features/plano-fases-19-20.md).
