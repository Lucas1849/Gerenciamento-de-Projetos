...
6. **Formas de resposta divergentes para `Projeto`** (observado na revisão das Fases 3–4): o `GET /projetos/` passou a embutir `equipe` (`ProjetoListaResposta`), mas o `PUT /projetos/{id}` devolvia o `ProjetoResposta` "cru". Escrever a resposta do PUT inteira de volta no estado da listagem apagaria a `equipe` silenciosamente — o frontend só não quebrava porque fazia merge parcial defensivo (`atualizarProjetoLocal` em `App.jsx` e o merge de `tap_assinado` em `PaginaProjeto.jsx`). **Prática de risco**: depender de merge defensivo no cliente para compensar endpoints que devolvem representações diferentes da mesma entidade. **Correção aplicada (05/07/2026)**: o PUT passou a devolver `ProjetoListaResposta` com a `equipe` derivada embutida, a mesma forma consumida pela listagem; os merges no frontend permanecem como defesa em profundidade. Regra daqui em diante: endpoints de escrita devolvem a mesma representação que a tela que consome o resultado.
