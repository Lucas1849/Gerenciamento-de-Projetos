# Catálogo de Serviços — Etapas Padrão

**Este documento precisa ser preenchido e validado com a diretoria antes de virar seed no banco (Fase 2 do plano de continuidade).** As etapas, dias úteis e descrições abaixo **não devem ser inventadas pela IA** — são informação operacional real da consultoria (ver ADR-005 em [../arquitetura/decisoes.md](../arquitetura/decisoes.md)).

Para cada serviço da cartela, preencher a tabela com as etapas padrão, na ordem em que normalmente acontecem. `Dias úteis esperados` é o prazo padrão daquela etapa (pode ser ajustado por projeto depois). `Descrição padrão` é o texto que já vem preenchido ao criar um projeto daquele serviço (editável).

Depois de preenchido e aprovado, os dados aqui viram o script `backend/app/seed_catalogo.py` (Fase 2).

---

## 1. Pesquisa de Mercado

| Ordem | Etapa | Dias úteis esperados | Descrição padrão |
|---|---|---|---|
|1|Alinhamento e Kick Off|10||
|2|Identificação do Problema|3||
|3|Objetivo Geral da Pesquisa|3||
|4|Definição do Público-alvo e Amostra|7||
|5|Análise de Concorrência|10||
|6|Análise de Tendências|5||
|7|Método de Pesquisa|6||
|8|Aplicação do Método de Pesquisa|20||
|9|Análise e Tabulação dos Resultados|10||

*Fonte: Proposta Esmalteria (bloco 4.1). Total: 74 dias úteis.*

## 2. Plano de Marketing

| Ordem | Etapa | Dias úteis esperados | Descrição padrão |
|---|---|---|---|
|1|Kick Off e Planejamento|5||
|2|Análise de Contexto|3||
|3|Análise de Setor|7||
|4|Análise de Recursos|5||
|5|Análise de Concorrentes e Vantagens Competitivas|6||
|6|Potenciais Parcerias|10||
|6|Jornada do Cliente|10||
|7|Definição do Público-alvo|5||
|8|Persona|10||
|9|Mix de Marketing|10||
|10|Análise Swot|5||
|11|Elaboração de Plano de Ação|10||
|12|Relatório Final|5||

*Fonte: Proposta Apoio Consultoria — Brambilla Planejados. Ordem 6 é entrega em bloco (Potenciais Parcerias + Jornada do Cliente, 10 dias úteis para as duas juntas). Total: 81 dias úteis.*

## 3. Plano de Comunicação

| Ordem | Etapa | Dias úteis esperados | Descrição padrão |
|---|---|---|---|
|1|Alinhamento e Kick Off|5||
|2|Contexto e Determinação de Objetivos|15||
|2|Análise Mercadológica|15||
|2|Público-Alvo|15||
|3|Planejamento de Metas|10||
|3|Orçamento de Comunicação|10||
|3|Composto/Mix Promocional|10||
|4|Seleção de Meios e Canais|20||
|4|Elaboração das Mensagens|20||
|5|Elaboração do Relatório Final|5||

*Fonte: Cópia de Proposta Ecocash. Ordens 2, 3 e 4 são entregas em bloco (etapas dentro de cada ordem compartilham o mesmo prazo). Total: 55 dias úteis.*

## 4. Plano de Negócios

| Ordem | Etapa | Dias úteis esperados | Descrição padrão |
|---|---|---|---|
|1|Alinhamento e Kick Off|5||
|2|Planejamento das Atividades|5||
|3|Análise de Setor|20||
|3|Análise de Mercado|20||
|4|Pesquisa de Mercado|30||
|5|Plano de Marketing|10||
|6|Plano Operacional|12||
|6|Recursos Humanos|12||
|7|Plano Financeiro|10||
|8|Relatório Final|3||

*Fonte: Proposta Filipe. Ordens 3 e 6 são entregas em bloco. Total: 95 dias úteis.*

## 5. Planejamento Estratégico

| Ordem | Etapa | Dias úteis esperados | Descrição padrão |
|---|---|---|---|
|1|Planejamento|5||
|2|Análise de Contexto|15||
|3|Definição de Propósito e Valores|5||
|4|Estabelecimento de Objetivos|5||
|5|Formulação de Estratégias|10||
|6|Plano de Ação|10||
|7|Relatório Final|5||

*Fonte: Planejamento Estratégico.pdf (arquivo padrão). Total: 55 dias úteis.*

## 6. Plano Operacional

| Ordem | Etapa | Dias úteis esperados | Descrição padrão |
|---|---|---|---|
|1|Alinhamento e Kick Off|5||
|2|Análise de Contexto|20||
|3|Definição de Metas e Indicadores|5||
|4|Layout e Fluxo de Processos|15||
|5|Organograma|10||
|6|Relatório|5||

*Fonte: Proposta Central Embalagens. Total: 60 dias úteis.*

## 7. Mapeamento de Processos

| Ordem | Etapa | Dias úteis esperados | Descrição padrão |
|1|Planejamento das Atividades|5||
|2|Análise de Contexto|10||
|3|Definição de metas e Indicadores|5||
|4|Estruturação dos Processos|20||
|5|Descrição de cargos|10|
|6|Relatório final|5||

## 8. Cliente Oculto

| Ordem | Etapa | Dias úteis esperados | Descrição padrão |
|---|---|---|---|
|1|Alinhamento e Kick Off|5||
|2|Definição de Objetivos|2||
|3|Definição dos Critérios de Avaliação|3||
|4|Orientação dos Consultores|5||
|5|Execução|10||
|6|Análise|5||
|7|Relatório Final|5||

*Fonte: Cliente Oculto.pdf (arquivo padrão). Etapa 5 (Execução) consta como "10 dias úteis (média mínima)" no original — pode variar por projeto. Total: 35 dias úteis.*

## 9. Valuation

*Serviço novo — ainda não validado com a diretoria, cronograma abaixo baseado na única proposta realizada até o momento (Proposta Midiática), pode não refletir o padrão final do serviço.*

| Ordem | Etapa | Dias úteis esperados | Descrição padrão |
|---|---|---|---|
|1|Kick-off e Alinhamento|5||
|2|Análise de Contexto|5||
|3|Análise de Setor|15||
|3|Análise de Mercado|15||
|4|Análise dos Intangíveis|15||
|5|Previsibilidade e Escalabilidade da Receita|15||
|6|Relatório Final|3||

*Fonte: Proposta Midiática ("Diagnóstico Estratégico para Valuation" / "Análise Estratégica para Geração de Valor"). Ordem 3 é entrega em bloco. Total: 58 dias úteis.*

---

## Aprovação

| Serviço | Validado por | Data |
|---|---|---|
| Pesquisa de Mercado | Aprovado | 04/07 |
| Plano de Marketing | Aprovado | 04/07 |
| Plano de Comunicação | Aprovado | 04/07 |
| Plano de Negócios | Aprovado | 04/07 |
| Planejamento Estratégico | Aprovado | 04/07 |
| Plano Operacional | Aprovado | 04/07 |
| Mapeamento de Processos | Aprovado | 04/07 |
| Cliente Oculto | Aprovado | 04/07 |
| Valuation | Aprovado | 04/07 |