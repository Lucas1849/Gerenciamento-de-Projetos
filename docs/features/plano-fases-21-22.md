...
> **Ambas as fases foram executadas em 10/07/2026** sob comando direto do responsável (ADRs 023–024 → "implementado"). Fase 21: dataset real gerado das planilhas com crosswalk resolvido, fluxo ADR-001 rodado — o wipe único cobriu também a pendência da Fase 20 —, `test_fase21.py` na suíte (67 testes). Fase 22: `GaleriaProjetos.jsx` no lugar do `KanbanFases.jsx` (removido), com os dois defaults propostos aceitos — chips de filtro por fase **incluídos** e faixa de cor **mantida**. Os ADRs estão registrados em [../arquitetura/decisoes.md](../arquitetura/decisoes.md). **O responsável respondeu as perguntas principais em 09/07/2026** (seedar os 72; pesquisa prevalece; interesse em orientar vira **coluna nova** — forma delegada; galeria com **3 cards por linha** no desktop) — respostas registradas na seção ao final. A pergunta de privacidade foi resolvida por verificação: **o repositório é público** (`Lucas1849/Gerenciamento-de-Projetos`), logo o dataset de professores fica em **arquivo local gitignorado**. **Seguem abertas** (não bloqueiam, têm default proposto): chips de filtro por fase (6) e faixa de cor no card (7). A execução só começa **mediante comando direto do responsável**.

| Fase | Entrega | Status |
|---|---|---|
| 21 | **Seed de professores orientadores**: `seed_professores.py` popula a tabela `Professor` com o cruzamento das duas planilhas da área | ✅ Executada (10/07/2026) |
| 22 | **Galeria de projetos** substitui o Kanban de fases dentro da gestão; fase vira badge no card + select na aba Visão Geral | ✅ Executada (10/07/2026) |
...
6. **(22) Filtro por fase na galeria** (chips com contagem acima do grid): ✅ resolvida pelo default na execução (10/07/2026) — **incluídos** (`.galeria-filtros`, toggle por chip). Remover é trivial se o responsável preferir.
7. **(22) Faixa de cor no card:** ✅ resolvida pelo default na execução (10/07/2026) — **mantida** (`.kanban-card-faixa` na cor da fase, junto do badge). Remover é um delete de linha se ele preferir.
