...
// Fase 8: modo 'estender' — prazo/data do bloco são exibidos read-only e a
// etapa adicionada os adota; o status continua individual.
...
export default function ModalBloco({ nomes, diasInicial, dataInicial, onConfirmar, onCancelar, modo = 'criar' }) {
  ...
  return (
    <div
      role="dialog"
      aria-modal="true"
      aria-label={estender ? 'Adicionar ao bloco de entrega' : 'Formar bloco de entrega'}
      style={{
    ...
    >
      ...
      >
        <h3 style={{ display: 'flex', alignItems: 'center', gap: 'var(--sp-8)', fontSize: 'var(--text-h3)', fontWeight: 700, marginBottom: 'var(--sp-12)' }}>
          <Link2 size={18} /> {estender ? 'Adicionar ao bloco de entrega' : 'Formar bloco de entrega'}
        </h3>
        <p style={{ fontSize: 'var(--text-body2)', color: 'var(--color-text-secondary)', marginBottom: 'var(--sp-8)' }}>
          {estender
            ? 'A etapa adota o prazo e a data de início do bloco; o status continua individual.'
        ...
        <div style={{ display: 'flex', gap: 'var(--sp-8)', justifyContent: 'flex-end' }}>
          ...
          <button
            ...
            onClick={() =>
              estender
                ? onConfirmar()
                : onConfirmar({ dias: Number(dias), dataInicio: dataInicio || null })
            }
          >
            {estender ? 'Adicionar ao bloco' : 'Formar bloco'}
          </button>
        </div>
      </div>
    </div>
